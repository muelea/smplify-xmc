...
def create_vert_tri_mat(faces, nv):
    mat = torch.zeros(nv, faces.shape[0])
...
def get_labeled_exif(path):
    ...
    try:
        ...
        for (key, val) in exif.items():
            ...
            if TAGS.get(key) == 'FocalLength':
                if len(val) == 2:
                    focal_length_mm = val[0]  / val[1]
                else:
                    focal_length_mm = val[0]
    except:
        labeled = {}
        focal_length_mm = ''
    return labeled, focal_length_mm
